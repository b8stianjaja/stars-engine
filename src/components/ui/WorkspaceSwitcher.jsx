...
const ArtistPanel = ({ children }) => {
  const { 
    ...
    artLayers, activeLayerId, setActiveLayer, addArtLayer, removeArtLayer, updateArtLayer, reorderArtLayer, duplicateArtLayer,
    studioTools, setStudioTool, studioView, setStudioView,
    timeline, setTimeline, transformMode, setTransformMode, undoArtLayer, pushUndoState, redoArtLayer,
  ...
  const handleClearLayer = () => {
    if (selectedEntityId && director.isCameraLocked) {
      useStore.getState().updateEntityData(selectedEntityId, { texture: null });
    } else if (activeLayerId) {
      pushUndoState();
      useStore.getState().updateArtLayerFrame(activeLayerId, timeline.currentFrame, null);
    }
  };
...
const DevPanel = ({ children }) => {
  const { entities, selectedEntityId, setSelectedEntity, updateEntityLogic, updateEntityData, removeEntity, duplicateEntity, sceneLogic, updateSceneLogic, scenes, activeSceneId } = useStore(useShallow(state => ({
          ...
          <button 
            onClick={() => { 
              if(window.confirm('FACTORY RESET: Wipe all entities from this scene to fix corrupted cache?')) { 
                useStore.setState({ entities: [], selectedEntityId: null }); 
              } 
            }} 
...
export function WorkspaceSwitcher({ children }) {
  const { 
    workspaceMode, toggleWorkspaceMode, isPlaying, togglePlay, exportSceneData,
    scenes, activeSceneId, createScene, switchScene, setInputKey,
    selectedEntityId, removeEntity, director,
    uiTheme, toggleTheme
  } = useStore(useShallow(state => ({
    ...
    exportSceneData: state.exportSceneData,
    scenes: state.scenes,
    activeSceneId: state.activeSceneId,
  ...
  // Sync theme to root element for global CSS vars
  useEffect(() => {
    document.documentElement.setAttribute('data-theme', uiTheme);
  }, [uiTheme]);

  // Global entity deletion shortcut
  useEffect(() => {
    ...
    <div className="ide-root" data-theme={uiTheme}>
      ...
      <div className="ide-header">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          ...
          <button onClick={exportSceneData} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1.25rem', borderRadius: '6px', cursor: 'pointer', fontSize: '0.8rem', background: 'var(--bg-panel)', border: '1px solid var(--border-subtle)', color: 'var(--text-main)', fontWeight: 600, transition: 'all 0.2s' }}>
